...
async def create_resident(
    data: ResidentCreate, service: ResidentService = Depends(get_resident_service)
):
...
async def list_residents(
    skip: int = 0, limit: int = 100, service: ResidentService = Depends(get_resident_service)
):
    return await service.list_residents(skip, limit)


@router.get("/{resident_id}", response_model=ResidentOut)
async def get_resident(
    resident_id: str, service: ResidentService = Depends(get_resident_service)
):
