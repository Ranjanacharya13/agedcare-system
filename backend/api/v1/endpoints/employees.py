...
async def create_employee(
    data: EmployeeCreate, service: CrudService = Depends(get_employee_service)
):
...
async def list_employees(
    skip: int = 0, limit: int = 100, service: CrudService = Depends(get_employee_service)
):
...
async def get_employee(
    employee_id: str, service: CrudService = Depends(get_employee_service)
):
...
async def update_employee(
    employee_id: str,
    data: EmployeeUpdate,
    service: CrudService = Depends(get_employee_service),
):
