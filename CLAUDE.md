...
The database schema must exist before anything beyond `/health` works. Run `backend/sql/000_schema.sql` once in the Supabase SQL editor — it creates all 26 tables and is safe to run against a database that already has some of them. Then create the first account:
